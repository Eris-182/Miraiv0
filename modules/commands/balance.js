...
module.exports.run = async function({ api, event, args, Currencies, utils }) {
	if (!args[0]) {
		...
		return api.sendMessage(`Số tiền bạn hiện đang có: ${money.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")} coin`, event.threadID);
	}
	else if (Object.keys(event.mentions).length == 1) {
		var mention = Object.keys(event.mentions)[0];
		const money = (await Currencies.getData(mention)).money;
		return api.sendMessage({
			body: `Số tiền của ${event.mentions[mention].replace("@", "")} hiện đang có là: ${money.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")} coin.`,
			mentions: [{
		...
		for (let value of mention) {
			...
			msg += (` - ${event.mentions[value].replace("@", "")}: ${data.money.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")} coin\n`);
		};

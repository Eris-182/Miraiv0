...
module.exports.run = async function({ api, event, args, Users }) {
  ...
  const Sex = x == 2 ? "Onii chan" : x == 1 ? "Onee chan" : "";
	...
	api.sendMessage(`Em sẽ nhắc ${Sex} sau: ${display}`, event.threadID, event.messageID);
	...
	body: `${(text) ? value + ", " + Sex +" đã để lại lời nhắn như sau:" + text : value + ", hình như bạn yêu cầu tôi nhắc bạn làm việc gì đó thì phải?"}`,
		mentions: [{
			tag: value,
			id: event.senderID
		}]
